...
from PyQt5.QtWidgets import QLabel,QMainWindow,QApplication,QPushButton,QTextBrowser,QLineEdit,QAction,QMessageBox,QMenuBar
    ...
    def run(self):
        if "playlist" in self.url:
            ...
            if(song_name=='Error' and song_artist=='Error'):
                ...
                while ('/' in file_name):
                    position = file_name.find('/')
                    file_name = file_name[:position] + ' ' + file_name[position +
                                                                    1:]
                if (self.path == ''):
...
class DownMusic(QMainWindow):
    ...
    def StartGui(self):
        ...
        self.bt3.setFont(QFont("微软雅黑", 10))
        ...
        self.setWindowTitle('网易云下载工具V1.2')
